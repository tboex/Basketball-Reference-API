...
class Team:
    ...
    def get_team_history_info(self):
        raise NotImplementedError
...
class TeamHistory(Team):
    def fetch_team(self):
        html = self.fetch_url()
        if html:
            history = self.clean_team_url(html)
            self.get_team_history_info(history)
    ...
    def clean_team_url(self, html):
        ...
        return soup.find('div', attrs={"class": "overthrow"})
...
class TeamSpecific(Team):
    def __init__(self, name, year, links):
        self.name = name
        self.team_headers = []
        self.team_stats = []
        self.team_links = links
    ...
    def fetch_team_specifics(self):
        html = self.fetch_url()
        if html:
            specifics = self.clean_team_url(html)

    def fetch_url(self):
        is_specific = self.from_file("specific")
        if is_specific == False:
            URL = "https://www.basketball-reference.com"
            URL = URL + self.team_links[self.specific_year]
            html = ""
            try:
                with requests.get(URL) as response:
                    # use whatever encoding as per the webpage
    ...
    def clean_team_url(self, html):
        ...
        try:
            self.get_roster(roster)
        except:
            print("No Roster found")
        try:
            self.get_totals(totals)
        except:
            print("No Totals Found")
        try:
            self.get_info_for_year(info)
        except:
            print("No info for year")
        try:
            self.get_assistant_staff(staff)
        except:
